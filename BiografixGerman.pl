...
#Create new sentences out of parentehical structures with biographical information. German version.
...
# eliminate space at the beginging and end of the string
$date =~ s/^\s+//;
$town=~ s/^\s+//;
$province =~ s/^\s+//;
$state =~  s/^\s+//;
...
#PRINT NEW SENTENCE
  ...
  print "am $date";
    ...
    print " geboren.\n";
	 ...
	 print "$town ist in $province.\n";
     }

    if(($state ne "") && ($province ne ""))  {
	
	print "$province ist in $state.\n";
     }

     if(($state ne "") && ($place4 ne ""))  {
	 
	print "$state ist in $place4.\n";
     }
...
#eliminate space at the end of the string
$date2 =~ s/^\s+//;
$town=~ s/^\s+//;
$province =~ s/^\s+//;
$state =~  s/^\s+//;
