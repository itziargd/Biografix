...
#Create new sentences out of parentehical structures with biographical information. Italian version.
...
#PRINT NEW SENTENCE
  ...
  print "el $date";
    ...
    print ".\n";
	 ...
	 print "$town è in $province.\n";
     }

    if(($state ne "") && ($province ne ""))  {
	
	print "$province è in $state.\n";
     }

     if(($state ne "") && ($place4 ne ""))  {
	 
	print "$state è in $place4.\n";
     }
